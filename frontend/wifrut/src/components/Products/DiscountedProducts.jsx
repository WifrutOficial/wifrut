...
function DiscountedProducts({ products, quantities, handleQuantityChange, handleAddToCart }) {
  const discountedProducts = products.filter((product) => Number(product.descuento) > 0);
    ...
    <>
      ...
      <div className={style.descuento}>
        <div className={style.container2}>
          {discountedProducts.map(({ _id, nombre, descuento, descripcion, tipoVenta, precioConDescuento, imagen }) => (
            <div key={_id} className={style.cartContainer}>
              ...
              <p className={style.priceUnit}>Precio con descuento: ${precioConDescuento || 0}</p>
              ...
              <button
                ...
                onClick={() =>
                  handleAddToCart({ _id, nombre, descuento, precioConDescuento, tipoVenta, cantidad: quantities[_id] || 0 })
                }
              >
                Añadir a carrito
              </button>
